<?php return function (\PDO $connection, int $id_shift): bool {
	...
	$shift_datetime = Tables\Shifts::select_datetime_from($connection, $id_shift);

	if(!Tables\Shifts::delete($connection, $id_shift))
		return false;

	$user_list_from_shift = Tables\ShiftUserMaps::select_all_with_id_shift($connection, $id_shift);

	foreach ($user_list_from_shift as $user) {

		$get_template_email_user_promote = include 'services/get_email_template.php';
		$email_template = $get_template_email_user_promote($connection, Tables\EmailTemplates::SHIFT_DELETE);
	...
	return Tables\ShiftUserMaps::delete_shift($connection, $id_shift);
};