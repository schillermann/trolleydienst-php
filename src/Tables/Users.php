...
class Users
{
    ...
    static function select_user(\PDO $connection, int $id_user): array
    {
        ...
        $stmt = $connection->prepare(
            ...
            language, note_admin, note_user, active, administrative,  updated_on, created_on
            FROM ' . self::TABLE_NAME . '
            WHERE id = :id'
        );
    ...
    static function update_profile(\PDO $connection, \App\Models\Profile $profile): bool
    {
        $stmt = $connection->prepare(
            ...
            note_user = :note_user,  updated_on = datetime("now", "localtime")
            WHERE id = :id'
        );
    ...
    static function update_user(\PDO $connection, \App\Models\User $user): bool
    {
        $stmt = $connection->prepare(
            ...
            language = :language, note_admin = :note_admin,  updated_on = datetime("now", "localtime")
            WHERE id = :id'
        );
    ...
    static function insert(\PDO $connection, \App\Models\User $user): bool
    {
        ...
        $stmt = $connection->prepare(
            ...
            (
                ...
                language, note_admin, active, administrative,  updated_on, created_on
            )
            VALUES (
                :username, :first_name, :last_name, :email, :password, :phone, :mobile, :congregation, :language,
                :note_admin, :active, :administrative, datetime("now", "localtime"), datetime("now", "localtime")
            )'
        );
